...
public class OnPluginMessage implements Listener {
    ...
    public OnPluginMessage(AetheriaCoreBungee main) {
        this.main = main;
    }
        ...
        if(subChannel.equals(Constants.subChannelName)) {
            ...
            try {
                ...
                if(action.equals(Constants.getState)) {
                    ...
                    if(AetheriaCoreBungee.getInstance().getAuthHandler().getAuthState(player.getUniqueId()) == null)
                        AetheriaCoreBungee.getInstance().getAuthHandler().changeState(player.getUniqueId(), defaultState);
                    ...
                    if(state != AetheriaCoreBungee.getInstance().getAuthHandler().getAuthState(player.getUniqueId()))
                        AetheriaCoreBungee.getInstance().getAuthHandler().changeState(player.getUniqueId(), state);
    ...
    public void sendResponse(ProxiedPlayer player, String action) {
        AuthHandler.AuthState state = AetheriaCoreBungee.getInstance().getAuthHandler().getAuthState(player.getUniqueId());
