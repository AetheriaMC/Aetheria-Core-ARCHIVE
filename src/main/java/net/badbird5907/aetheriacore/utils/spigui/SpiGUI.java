package net.badbird5907.aetheriacore.utils.spigui;

import net.badbird5907.aetheriacore.utils.spigui.buttons.SGButton;
import net.badbird5907.aetheriacore.utils.spigui.item.ItemBuilder;
import net.badbird5907.aetheriacore.utils.spigui.menu.SGMenuListener;
import net.badbird5907.aetheriacore.utils.spigui.menu.SGOpenMenu;
import net.badbird5907.aetheriacore.utils.spigui.pagination.SGPaginationButtonBuilder;
...
public class SpiGUI {
	...
	private SGPaginationButtonBuilder defaultPaginationButtonBuilder = (type, inventory) -> {
		switch (type) {
			case PREV_BUTTON:
				return (inventory.getCurrentPage() > 0) ? new SGButton(new ItemBuilder(ARROW).name("&a&l\u2190 Previous Page").lore("&aClick to move back to", "&apage " + inventory.getCurrentPage() + ".").build()).withListener(event -> {
					event.setCancelled(true);
					inventory.previousPage(event.getWhoClicked());
				}) : null;
			case CURRENT_BUTTON:
				return new SGButton(new ItemBuilder(NAME_TAG).name("&7&lPage " + (inventory.getCurrentPage() + 1) + " of " + inventory.getMaxPage()).lore("&7You are currently viewing", "&7page " + (inventory.getCurrentPage() + 1) + ".").build()).withListener(event -> event.setCancelled(true));
			case NEXT_BUTTON:
				return (inventory.getCurrentPage() < (inventory.getMaxPage() - 1)) ? new SGButton(new ItemBuilder(ARROW).name("&a&lNext Page \u2192").lore("&aClick to move forward to", "&apage " + (inventory.getCurrentPage() + 2) + ".").build()).withListener(event -> {
					event.setCancelled(true);
	...
	public List<SGOpenMenu> findOpenWithTag(String tag) {
		...
		for (Player player : plugin.getServer().getOnlinePlayers()) {
			player.getOpenInventory().getTopInventory(); // ...if that player has an open inventory with a top inventory...
			Inventory topInventory = player.getOpenInventory().getTopInventory(); // ...get that top inventory.
