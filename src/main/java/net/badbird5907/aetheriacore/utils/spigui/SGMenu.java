...
import static net.badbird5907.aetheriacore.utils.spigui.pagination.SGPaginationButtonType.forSlot;
...
public class SGMenu implements InventoryHolder {
	...
	public SGButton getButton(int slot) {
		return ((slot < 0) || (slot > getHighestFilledSlot())) ? null : items.get(slot);
	}
	...
	public SGButton getButton(int page, int slot) {
		return ((slot < 0) || (slot > getPageSize())) ? null : getButton((page * getPageSize()) + slot);
	...
	public int getHighestFilledSlot() {
		int slot = 0;
		for (int nextSlot : items.keySet()) if (items.get(nextSlot) != null && nextSlot > slot) slot = nextSlot;
		return slot;
	}
	...
	public void refreshInventory(HumanEntity viewer) {
		...
		String newName = name.replace("{currentPage}", valueOf(currentPage + 1)).replace("{maxPage}", valueOf(getMaxPage()));
	...
	public Inventory getInventory() {
		...
		if (isAutomaticPaginationEnabled() != null) isAutomaticPaginationEnabled = isAutomaticPaginationEnabled();
		boolean needsPagination = getMaxPage() > 0 && isAutomaticPaginationEnabled;
		Inventory inventory = createInventory(this, ((needsPagination) ? getPageSize() + 9 : getPageSize()), name.replace("{currentPage}", valueOf(currentPage + 1)).replace("{maxPage}", valueOf(getMaxPage())));
		...
		if (needsPagination) {
			...
			if (getPaginationButtonBuilder() != null) paginationButtonBuilder = getPaginationButtonBuilder();
			int pageSize = getPageSize();
			for (int i = pageSize; i < pageSize + 9; i++) {
				SGButton paginationButton = paginationButtonBuilder.buildPaginationButton(forSlot(i - pageSize), this);
