...
public enum XMaterial {
	...
	static {
		stream(VALUES).forEach(VALUE -> NAMES.put(VALUE.name(), VALUE));
	}
	...
	private static XMaterial requestOldXMaterial(@Nonnull String name, byte data) {
		...
		for (XMaterial material : VALUES)
			if (((data == UNKNOWN_DATA_VALUE) || (data == material.data)) && material.anyMatchLegacy(name)) { // Not using material.name().equals(name) check is intended.
	...
	private static Optional<XMaterial> matchXMaterialWithData(@Nonnull String name) {
		...
		if (index != -1) {
			...
			try {
				...
				byte data = (byte) parseInt(StringUtils.deleteWhitespace(name.substring(index + 1)));
	...
	protected static Optional<XMaterial> matchDefinedXMaterial(@Nonnull String name, byte data) {
		...
		if (oldXMaterial == null) {
			...
			return data > 0 && name.endsWith("MAP") ? of(FILLED_MAP) : empty();
		}
	...
	public static Optional<XMaterial> matchXMaterial(int id, byte data) {
		...
		for (XMaterial materials : VALUES)
			if ((materials.data == data) && (materials.getId() == id)) return of(materials);
		return empty();
	}
