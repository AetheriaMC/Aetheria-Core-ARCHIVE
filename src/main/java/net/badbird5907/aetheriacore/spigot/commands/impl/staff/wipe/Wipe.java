package net.badbird5907.aetheriacore.spigot.commands.impl.staff.wipe;
...
public class Wipe implements CommandExecutor {
        ...
        if(sender.hasPermission(Permission.WIPE.node)){
            if(args.length > 0){
                ...
                for(int i=0; i < args.length; i++){
                    if(args[i].equalsIgnoreCase("--noconfirm")){
                        WipePlayer(sender, target, args);
                        break;
                    }
                }
                if(sender instanceof Player){
                    new ConfirmGUI().open(((Player) sender).getPlayer(), target.getName(), args);
                }else{
                    WipePlayer(sender, target, args);
                }
            }
    ...
    public void WipePlayer(CommandSender sender, OfflinePlayer target, String[] args){
