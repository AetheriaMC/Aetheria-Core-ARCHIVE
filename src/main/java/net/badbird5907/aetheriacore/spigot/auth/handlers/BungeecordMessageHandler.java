...
public class BungeecordMessageHandler implements PluginMessageListener {
    ...
    public BungeecordMessageHandler(AetheriaCore main) {
        this.main = main;
    }
    ...
    public void getBungeeCordAUthState(UUID uuid) {
        ...
        try {
            ...
            msgOut.writeUTF(TwoFactorAuthentication.getInstance().getAuthHandler().getAuthState(uuid).name()); // Default auth state
        } catch(IOException e) {
            e.printStackTrace();
        }
        ...
        if(subChannel.equals(Constants.subChannelName)) {
            ...
            try {
                ...
                if(action.equals(Constants.getState)) {
                    ...
                    TwoFactorAuthentication.getInstance().getAuthHandler().changeState(playerUUID, state);
                } else if(action.equals(Constants.setState)) {
                    state = AuthHandler.AuthState.valueOf(msgIn.readUTF());
                    if(state != TwoFactorAuthentication.getInstance().getAuthHandler().getAuthState(playerUUID)) {
                        TwoFactorAuthentication.getInstance().getAuthHandler().changeState(playerUUID, state);
                    }
                }
            } catch (IOException e) {
